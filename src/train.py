...
def parse_args():
    ...
    # New parser
    parser = ArgumentParser(description='PyTorch implementation of CDF-UIE')
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(TrainData(params.train_size, params.train_dir), batch_size=params.batch_size,
                              shuffle=True, num_workers=0)
    valid_loader = DataLoader(ValData_train(params.valid_dir), batch_size=1, shuffle=False, num_workers=0)
    CDF_UIE = CDF(params, trainable=True)
    CDF_UIE.train(train_loader, valid_loader)
